...
export type TabType = "digitar" | "arquivo";

interface EssayWritingBoxProps {
  essayText: string;
  setEssayText: (text: string) => void;
}

interface ExtrairTextoResponse {
  texto_extraido?: string;
}

export function EssayWritingBox({ essayText, setEssayText }: EssayWritingBoxProps) {
  /* Aba principal: "arquivo" | "digitar" */
  const [mainTab, setMainTab] = useState<TabType>("digitar");
  ...
  async function handleEssayUpload() {
    if (mainTab === "arquivo") {
      ...
      try {
        const response = await api.post<ExtrairTextoResponse>("ai/extrair_texto/", formData);
